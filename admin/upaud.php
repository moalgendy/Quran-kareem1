...
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $se =mysqli_query($con , "select * from `audios` where `id` = $id");
...
if(isset($_POST['add'])){ 
    $name = $_POST['name'];
     $upname = mysqli_query($con , "update `audios` set `name` = '$name' where `id` = '$id'");
     if($upname){
    ...
    if(move_uploaded_file($_FILES['m']['tmp_name'],$tp)){
        $video = $_FILES['m']['name'];
        $upvideo = mysqli_query($con , "update `audios` set `audios` = '$video' where `id` = '$id'");
        if($upvideo){
         header("location: audios_sel.php");
        }
    ...
    if(move_uploaded_file($_FILES['m']['tmp_name'],$tp)){
        ...
        $up = mysqli_query($con , "update `audios` set `name` = $namee , `audios` = $videoo where `id` = '$id'");
